...
export function computeCoverageMetrics(
  ...
  const denominator = documented; // scoped to current Archive snapshot only
  ...
  const percent =
    denominator > 0 ? Math.round((documented / denominator) * 10000) / 100 : null;

  // Completeness overclaim: 100% without an explicit scoped denominator label is forbidden.
  ...
  const user_facing_summary = denominator
    ? `${documented.toLocaleString()} records documented in the current Archive snapshot. Coverage is not a claim of all species known to science.`
    : 'No denominator — percentage coverage claim withheld.';
  ...
  const snapshot: ArchiveCoverageSnapshot = {
    ...
    scope_filters: { semantics: opts.coverage_semantics },
...
export function coveragePercentAfterAddingUndocumented(
  priorDocumented: number,
  denominator: number,
): number | null {
  if (denominator <= 0) return null;
  return Math.round(((priorDocumented + 1) / denominator) * 10000) / 100;
}
