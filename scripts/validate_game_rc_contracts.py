...
def main() -> int:
    ...
    if visual.get("VISUAL_MODEL_REVIEW") != "UNAVAILABLE":
        errors.append("VISUAL harness must stay UNAVAILABLE without live pixels")
    if visual.get("status") not in {"HARNESS_READY", "CAPTURE_DEFERRED", "HISTORICAL_ONLY", "UNAVAILABLE"}:
        errors.append("visual harness status invalid")

    global_mode = next((m for m in content.get("modes", []) if m.get("id") == "global_species_knowledge_base"), None)
    if global_mode and global_mode.get("status") != "OPEN":
        errors.append("global coverage must stay OPEN")
