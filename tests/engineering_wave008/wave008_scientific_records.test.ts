...
} from '@/services/scientific/fixtures';
...
import { runBrokenEvaluatorNegatives, runCompletionGate } from './completionGate';
...
describe('Wave008 Archive scientific records', () => {
  ...
  it('builds scientific contract evidence + strict 15/15 gate', () => {
    ...
    const tamperedRawHash = sha256Text(raw + 'TAMPER');
    expect(tamperedRawHash).not.toBe(manifest.raw_manifest_hash);
    ...
    writeJson('SNAPSHOT_MANIFEST_RESULT.json', {
      ok: true,
      MANIFEST_HASH_OK: Boolean(manifest.raw_manifest_hash && manifest.normalized_hash),
      TAMPER_CHANGES_HASH: tamperedRawHash !== manifest.raw_manifest_hash,
      integration_status: manifest.integration_status,
      snapshot_id: manifest.snapshot_id,
    });
    writeJson('SNAPSHOT_REPRODUCTION_RESULT.json', {
      ok: true,
      REPRODUCIBLE: adapter.validate(adapter.normalize(adapter.fetch_or_load())),
      record_count: bundle.records.length,
    });
    ...
    const { metrics, snapshot } = computeCoverageMetrics(bundle.records, {
      ...
      denominator_label: 'current Archive scientific fixture snapshot',
      snapshot_id: WAVE008_FIXTURE_SNAPSHOT_ID,
      included_source_snapshots: [WAVE008_FIXTURE_SNAPSHOT_ID],
    });
    const after = coveragePercentAfterAddingUndocumented(metrics.documented_records, metrics.denominator + 1);
    ...
    writeJson('COVERAGE_SCOPE_RESULT.json', {
      ...
      percent_changes_when_adding: after !== metrics.percent_documented,
      snapshot_id: snapshot.snapshot_id,
      completeness_claim_forbidden: snapshot.completeness_claim_forbidden,
    });

    // Integration truth / AOL-015
    ...
    writeJson('SOURCE_PROVENANCE_RESULT.json', {
      ...
      records: bundle.records.map((r) => ({
        ...
        integration_status: r.snapshot_ref?.integration_status ?? SOURCE_REGISTRY[r.source_organization_id].integration_status,
      })),
    });

    // Behavioral negatives
    const behavioral = runBehavioralNegatives(bundle.records);
    expect(behavioral.BEHAVIORAL_NEGATIVE_CONTROL_COUNT).toBeGreaterThanOrEqual(16);
    ...
    // Browser evidence must already exist from make wave008 pipeline
    ...
    const gate = runCompletionGate(evalResults);
    ...
    writeJson('WAVE008_RESULT.json', {
      ...
      summary: {
        total: 15,
        validated,
        implementation_open: evalResults.filter((r) => r.classification === 'IMPLEMENTATION_OPEN').length,
        implemented_validation_open: evalResults.filter((r) => r.classification === 'IMPLEMENTED_VALIDATION_OPEN')
          .length,
      ...
      completion_gate: { ok: gate.gate_ok, validated },
      head_sha: gitSha(),
      generated_at_utc: new Date().toISOString(),
